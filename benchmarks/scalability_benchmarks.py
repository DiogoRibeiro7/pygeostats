...
class ScalabilityBenchmark:
    ...
    def generate_synthetic_data(
        ...
        if anisotropic:
            ...
            if n_samples > 5000:
                ...
                sample_values = np.random.multivariate_normal(np.zeros(n_sample), cov_matrix)
    ...
    @memory_profile
    def benchmark_variogram_computation(self, n_samples_list: List[int]) -> pd.DataFrame:
        ...
        for n_samples in n_samples_list:
            ...
            for approach_name, test_func in approaches.items():
                ...
                results.append({
                    ...
                    'predictions_computed': len(predictions) if predictions is not None else 0
                })
    ...
    def benchmark_distance_computations(self, n_samples_list: List[int]) -> pd.DataFrame:
        ...
        for n_samples in n_samples_list:
            ...
            for approach_name, test_func in approaches.items():
                ...
                try:
                    ...
                    success = True
                    result_shape = distance_result.shape if hasattr(distance_result, 'shape') else len(distance_result)
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, pd.DataFrame]:
        ...
        print(f"System memory: {self.memory_manager.get_memory_info()['system_total_gb']:.1f} GB")
        print()
        ...
        kriging_sizes = [size for size in test_sizes if size <= 50000]  # Limit for kriging
        ...
        for n_samples in n_samples_list:
            ...
            try:
                ...
                if dense_memory_estimate < 8.0:  # Only if less than 8GB
                    ...
                    memory_after = self.memory_manager.get_memory_info()
                    dense_memory_actual = memory_after['process_gb'] - memory_before['process_gb']
            ...
            try:
                ...
                memory_after = self.memory_manager.get_memory_info()
                sparse_memory_actual = memory_after['process_gb'] - memory_before['process_gb']
            ...
            try:
                ...
                memory_after = self.memory_manager.get_memory_info()
                memmap_memory_actual = memory_after['process_gb'] - memory_before['process_gb']
    ...
    def _plot_performance_scaling(self, results: Dict[str, pd.DataFrame]) -> None:
        ...
        if 'variogram' in results:
            ...
            axes[0, 0].loglog(successful['n_samples'], successful['variogram_time_s'], 
                            'o-', label='Empirical Variogram')
            
            streaming = df[df['streaming_success']]
            if not streaming.empty:
                axes[0, 0].loglog(streaming['n_samples'], streaming['streaming_time_s'], 
                                's-', label='Streaming Variogram')
        ...
        if 'kriging' in results:
            ...
            for approach in df['approach'].unique():
                ...
                if not approach_data.empty:
                    axes[0, 1].loglog(approach_data['n_samples'], approach_data['time_s'],
                                    'o-', label=approach.replace('_', ' ').title())
        ...
        if 'distances' in results:
            ...
            for approach in df['approach'].unique():
                approach_data = df[(df['approach'] == approach) & df['success']]
                if not approach_data.empty:
                    axes[1, 0].loglog(approach_data['n_samples'], approach_data['time_s'],
                                    'o-', label=approach.replace('_', ' ').title())
        ...
        if 'memory' in results:
            ...
            if not valid_dense.empty:
                axes[1, 1].loglog(valid_dense['n_samples'], valid_dense['dense_memory_actual_gb'],
                                'o-', label='Dense Matrix')
            
            if not valid_sparse.empty:
                axes[1, 1].loglog(valid_sparse['n_samples'], valid_sparse['sparse_memory_actual_gb'],
                                's-', label='Sparse Matrix')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'performance_scaling.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_memory_usage(self, results: Dict[str, pd.DataFrame]) -> None:
        ...
        axes[0, 0].loglog(df['n_samples'], df['coords_size_gb'], 'o-', label='Coordinates')
        axes[0, 0].loglog(df['n_samples'], df['values_size_gb'], 's-', label='Values')
        
        valid_dense = df.dropna(subset=['dense_memory_actual_gb'])
        if not valid_dense.empty:
            axes[0, 0].loglog(valid_dense['n_samples'], valid_dense['dense_memory_actual_gb'],
                            '^-', label='Dense Matrix')
        ...
        if not valid_sparsity.empty:
            axes[0, 1].semilogx(valid_sparsity['n_samples'], valid_sparsity['sparsity_ratio'],
                              'o-', color='red')
            axes[0, 1].set_xlabel('Number of Samples')
        ...
        valid_comparison = df.dropna(subset=['dense_memory_actual_gb', 'sparse_memory_actual_gb'])
        if not valid_comparison.empty:
            memory_ratio = valid_comparison['sparse_memory_actual_gb'] / valid_comparison['dense_memory_actual_gb']
            axes[1, 0].semilogx(valid_comparison['n_samples'], memory_ratio, 'o-', color='green')
            axes[1, 0].set_xlabel('Number of Samples')
        ...
        valid_estimate = df.dropna(subset=['dense_memory_estimate_gb', 'dense_memory_actual_gb'])
        if not valid_estimate.empty:
            axes[1, 1].loglog(valid_estimate['dense_memory_estimate_gb'], 
                            valid_estimate['dense_memory_actual_gb'], 'o')
            
            # Add 1:1 line
            min_val = min(valid_estimate['dense_memory_estimate_gb'].min(),
                         valid_estimate['dense_memory_actual_gb'].min())
            max_val = max(valid_estimate['dense_memory_estimate_gb'].max(),
    ...
    def _plot_approach_comparison(self, results: Dict[str, pd.DataFrame]) -> None:
        ...
        pivot_time = successful_results.pivot(index='n_samples', columns='approach', values='time_s')
        
        for approach in pivot_time.columns:
            if not pivot_time[approach].isna().all():
                axes[0].loglog(pivot_time.index, pivot_time[approach], 'o-', 
        ...
        pivot_memory = successful_results.pivot(index='n_samples', columns='approach', values='memory_used_gb')
        
        for approach in pivot_memory.columns:
            if not pivot_memory[approach].isna().all():
                axes[1].loglog(pivot_memory.index, pivot_memory[approach], 's-',
                              label=approach.replace('_', ' ').title())
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'approach_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_gpu_comparison(self, results: Dict[str, pd.DataFrame]) -> None:
        ...
        for approach in cpu_results['approach'].unique():
            approach_data = cpu_results[(cpu_results['approach'] == approach) & cpu_results['success']]
            if not approach_data.empty:
                axes[0].loglog(approach_data['n_samples'], approach_data['time_s'],
                              'o-', label=f'CPU: {approach.replace("_", " ").title()}')
        
        gpu_successful = gpu_results[gpu_results['success']]
        if not gpu_successful.empty:
            axes[0].loglog(gpu_successful['n_samples'], gpu_successful['time_s'],
                          's-', color='red', label='GPU Accelerated')
        ...
        if not gpu_successful.empty and not cpu_results.empty:
            ...
            merged = gpu_successful.merge(cpu_best, on='n_samples', suffixes=('_gpu', '_cpu'))
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'gpu_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def generate_report(self, results: Dict[str, pd.DataFrame]) -> str:
        ...
        if 'variogram' in results:
            ...
            report_lines.extend([
                f"### Variogram Computation",
                f"- Maximum dataset size tested: {max_samples:,} samples",
                f"- Streaming variogram enables processing of 1M+ point datasets",
                ""
            ])
        ...
        if 'kriging' in results:
            ...
            if not successful.empty:
                ...
                best_approach = successful.loc[successful['time_s'].idxmin(), 'approach']
                
                report_lines.extend([
                    f"### Kriging Performance",
        ...
        if 'memory' in results:
            ...
            if not valid_comparison.empty:
                sparse_beneficial = valid_comparison[
                    valid_comparison['sparse_memory_actual_gb'] < valid_comparison['dense_memory_actual_gb']
                ]
                if not sparse_beneficial.empty:
                    min_sparse_samples = sparse_beneficial['n_samples'].min()
                    report_lines.append(f"- Use sparse matrices for datasets > {min_sparse_samples:,} samples")
            
            # Memory-mapped recommendations
            large_datasets = df[df['n_samples'] >= 10000]
            if not large_datasets.empty:
                report_lines.append("- Use memory-mapped arrays for datasets > 10,000 samples")
                report_lines.append("- Enable GPU acceleration when available for 5,000+ samples")
