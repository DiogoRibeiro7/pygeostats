...
def generate_sample_data(n_samples=100):
    ...
    values = np.random.multivariate_normal(mean=np.zeros(n_samples), cov=covariance)
...
def main():
    ...
    emp_vario = EmpiricalVariogram(
        known_coords, known_values, max_distance=5.0, n_bins=15
    )
    emp_vario.compute()
    ...
    print(
        f"   Prediction range: [{np.min(predictions):.3f}, {np.max(predictions):.3f}]"
    )
    ...
    ax = axes[0, 1]
    scatter = ax.scatter(
        known_coords[:, 0], known_coords[:, 1], c=known_values, cmap="viridis", s=50
    )
    ...
    ax.scatter(known_coords[:, 0], known_coords[:, 1], c="blue", s=20, alpha=0.8)
