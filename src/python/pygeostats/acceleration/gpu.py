# src/python/pygeostats/acceleration/gpu.py
...
class GPUAccelerator:
    ...
    def _direct_distances_gpu(
        ...
        try:
            ...
            warnings.warn(
                "GPU out of memory, falling back to CPU", UserWarning, stacklevel=2
            )
            return self._euclidean_distances_cpu(coords1, coords2)

    def _chunked_distances_gpu(
        self, coords1: np.ndarray, coords2: np.ndarray, chunk_size: int, symmetric: bool
    ) -> np.ndarray:
        ...
        try:
            ...
            warnings.warn(
                "GPU out of memory during chunked computation, falling back to CPU",
                UserWarning,
                stacklevel=2,
            )
            return self._euclidean_distances_cpu(coords1, coords2)

    def _euclidean_distances_cpu(
        self, coords1: np.ndarray, coords2: Optional[np.ndarray]
    ) -> np.ndarray:
    ...
    def variogram_distances_gpu(
        self, coords: np.ndarray, max_pairs: int = 1_000_000, batch_size: int = 50_000
    ...
    def _subsample_pairs_gpu(
        self, coords: np.ndarray, max_pairs: int, batch_size: int
    ) -> Tuple[np.ndarray, np.ndarray]:
...
def get_gpu_accelerator(
    enable_gpu: bool = True, device_id: Optional[int] = None
) -> GPUAccelerator:
