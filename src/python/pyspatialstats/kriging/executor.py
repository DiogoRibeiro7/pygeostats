...
class ParallelKrigingExecutor:
    ...
    def predict_parallel(
        ...
        print(f"Parallel kriging: {n_pred:,} predictions using {self.n_workers} workers")
    ...
    def _predict_spatial(
        ...
        if prediction_coordinates.shape[1] != 2:
            warnings.warn("Spatial tiling only supports 2D coordinates, falling back to chunking")
            return self._predict_chunked(kriging_model, prediction_coordinates, return_variance)
        ...
        if self.execution_method == "sequential":
            ...
            with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
                ...
                for tile_coords, tile_indices in tile_assignments:
                    if len(tile_coords) > 0:
                        future = executor.submit(worker_func, (tile_coords, tile_indices))
                        futures.append(future)
        ...
        return self._combine_spatial_results(results, len(prediction_coordinates), return_variance)
    ...
    def _predict_adaptive(
        ...
        if n_dims == 2 and n_pred > 10000:
            ...
            return self._predict_spatial(kriging_model, prediction_coordinates, return_variance)
        else:
    ...
    def _assign_points_to_tiles(
        ...
        for tile_bounds in tiles:
            ...
            mask = (
                (coordinates[:, 0] >= xmin) & (coordinates[:, 0] <= xmax) &
    ...
    def _collect_results_with_progress(self, futures, total_tasks: int):
        ...
        for future in as_completed(futures):
            try:
                ...
                warnings.warn(f"Task failed: {str(e)}")
                results.append(None)
    ...
    def _combine_spatial_results(
        ...
        if np.any(missing_mask):
            warnings.warn(f"{np.sum(missing_mask)} predictions failed and are set to NaN")
    ...
    def estimate_computation_time(
        ...
        parallel_time = (
            sequential_time / (self.n_workers * parallelization_efficiency)
        )
...
def _predict_spatial_tile_worker(
    tile_data: Tuple[np.ndarray, np.ndarray],
    kriging_model,
    return_variance: bool
        ...
        for i in range(start_idx, n_pred):
            ...
            if (i + 1) % self.checkpoint_interval == 0:
                self._save_checkpoint(
                    i + 1, predictions, variances, checkpoint_file
                )
