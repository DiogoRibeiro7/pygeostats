# src/python/pyspatialstats/variogram/initialization.py
...
def _validate_parameter_recovery() -> None:
    base_major = 0.45
    base_minor = 0.22
    sill = 1.05
    nugget = 0.05
    ...
    for angle in angles:
        ...
        ci = np.vstack((gamma - 0.05, gamma + 0.05))
        directional_results[angle] = DirectionalResult(
            angle=angle,
    ...
    initializer = RangeInitializer(directional_results)
    result = initializer.estimate()
    ...
    ensemble = InitializationEnsemble(coords_grid, directional_results)
    ensemble_result = ensemble.run()
    assert ensemble_result.quality > 0.2
    assert ensemble_result.top_candidates

    isotropic_coords = np.random.default_rng(0).uniform(-1.0, 1.0, size=(40, 2))
    isotropic_results = {angle: directional_results[angle] for angle in angles}
    isotropic_ensemble = InitializationEnsemble(isotropic_coords, isotropic_results)
    iso_result = isotropic_ensemble.run()
    assert iso_result.ratio >= 1.0


if __debug__:
    _validate_parameter_recovery()
