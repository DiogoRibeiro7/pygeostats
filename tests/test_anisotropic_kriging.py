...
class TestAnisotropicKriging:
        ...
        for i in range(n):
            for j in range(n):
                ...
                if i == j:
                    ...
                    gamma = self.nugget + (self.sill - self.nugget) * (
                        1 - np.exp(-aniso_dist)
                    )
    ...
    def test_anisotropic_distance_calculation(self):
        ...
        expected_dist = np.sqrt(
            (cos_45 / self.range_major) ** 2 + (cos_45 / self.range_minor) ** 2
        )
    ...
    def test_anisotropy_info(self):
        ...
        assert info["is_anisotropic"]  # ratio > 1.2
...
class TestDirectionalVariogramIntegration:
    ...
    def test_parameter_recovery_accuracy(self):
        ...
        ratio_error = (
            abs(recovered_ratio - self.true_params["ratio"]) / self.true_params["ratio"]
        )
...
class TestAnisotropicVsIsotropicComparison:
    ...
    def test_anisotropic_vs_isotropic_performance(self):
        ...
        test_indices = np.random.choice(
            n_samples, size=min(15, n_samples), replace=False
        )
        ...
        assert (
            aniso_rmse <= iso_rmse * 1.1
        ), "Anisotropic kriging should not be much worse"
        ...
        assert (
            prediction_diff > 0.01
        ), "Anisotropic and isotropic predictions should differ"
    ...
    def test_uncertainty_quantification(self):
        ...
        inside_mask = np.array(
            [_point_in_hull(point, self.coords[hull.vertices]) for point in pred_coords]
        )
        ...
        if np.any(inside_mask) and np.any(~inside_mask):
            ...
            assert (
                var_outside >= var_inside * 0.8
            ), "Extrapolation variance should be higher"
    ...
    def test_directional_dependence(self):
        ...
        assert (
            variance_ratio >= 0.8
        ), f"Expected directional variance difference, got ratio {variance_ratio:.2f}"
